...
import type { User, DocumentItem } from '../types';
...
const fetchDocuments = async (userRole: User['role']): Promise<DocumentItem[]> => {
  ...
  const allDocs: DocumentItem[] = [
    ...
    if (userRole === 'Delegate') {
        return allDocs.filter(doc => doc.status === 'Approved'); // Simple filter for delegates
    }
    return allDocs;
};
  ...
  useEffect(() => {
   ...
   const handleActionPress = (document: DocumentItem, action: 'View' | 'Approve' | 'Reject' | 'Delete') => {
     ...
     if (action === 'Approve' || action === 'Reject' || action === 'Delete') {
        ...
        // Simulate state update locally for prototype
     ...
     // For View/Download, you'd use Linking API or a WebView in a real app
   };
   ...
   const getStatusBadge = (status: DocumentItem['status']) => {
        ...
        switch (status) {
            ...
            case 'Pending Approval':
                style.push(globalStyles.badgeSecondary);
  ...
  return (
    <View style={globalStyles.container}>
      <Text style={typography.h1}>Documents</Text>
       {user?.role !== 'Chairman' && ( // Allow Delegate and Director to upload
            <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  uploadButton: {
      ...
      flexDirection: 'row', // Add icon to button
      alignItems: 'center',
